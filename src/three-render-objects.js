...
export default Kapsule({
  ...
  methods: {
    ...
    zoomToFit: function (
      ...
      return this.fitToBbox(
        this.getBbox(...bboxArgs),
        transitionDuration,
        padding
      );
    },
  ...
  init(
    ...
    state.container.appendChild(
      (state.navInfo = document.createElement("div"))
    );
      ...
      state.container.addEventListener(
        ...
        (ev) => {
          ...
          if (state.enablePointerInteraction) {
            ...
            state.toolTipElem.style.transform = `translate(-${
              (state.pointerPos.x / state.width) * 100
            }%, ${
              // flip to above if near bottom
              state.height - state.pointerPos.y < 100
                ? "calc(-100% - 8px)"
                : "21px"
            })`;
          }
    ...
    state.renderer = new three.WebGLRenderer(
      Object.assign({ antialias: true, alpha: true }, rendererConfig)
    );
    ...
    state.postProcessingComposer.addPass(
      new ThreeRenderPass(state.scene, state.camera)
    ); // render scene as first pass
  ...
  update(state, changedProps) {
    ...
    if (
      state.width &&
      state.height &&
      (changedProps.hasOwnProperty("width") ||
    ...
    if (changedProps.hasOwnProperty("skyRadius") && state.skyRadius) {
      state.controls.hasOwnProperty("maxDistance") &&
        ...
        (state.controls.maxDistance = Math.min(
    ...
    if (changedProps.hasOwnProperty("backgroundColor")) {
      ...
      state.renderer.setClearColor(
        new three.Color(opacify(1, state.backgroundColor)),
        alpha
      );
    }
    ...
    changedProps.hasOwnProperty("showNavInfo") &&
      (state.navInfo.style.display = state.showNavInfo ? null : "none");
